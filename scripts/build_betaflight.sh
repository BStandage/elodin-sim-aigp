...
if [ ! -f "$BETAFLIGHT_DIR/Makefile" ]; then
    ...
    echo "Run (from repo root): bash scripts/fetch_betaflight.sh"
    exit 1
fi
...
case "${1:-build}" in
    ...
    build|"")
        ...
        if grep -q "^//#define ENABLE_SIMULATOR_GYROPID_SYNC 1" "$TARGET_H"; then
            ...
            echo "Enabling legacy SIMULATOR_GYROPID_SYNC for lockstep mode..."
            sed -i.bak 's|^//#define SIMULATOR_GYROPID_SYNC|#define SIMULATOR_GYROPID_SYNC|' "$TARGET_H"
            rm -f "${TARGET_H}.bak"
        elif grep -q "^#define ENABLE_SIMULATOR_GYROPID_SYNC 1" "$TARGET_H" || grep -q "^#define SIMULATOR_GYROPID_SYNC" "$TARGET_H"; then
            echo "Simulator GYROPID sync already enabled."
        else
            echo "Warning: Could not find simulator GYROPID sync define in target.h"
        fi
        ...
        if [[ "$(uname)" == "Darwin" ]]; then
            STUBS_C="$(mktemp "${TMPDIR:-/tmp}/agp-macos-sitl-stubs.XXXXXX.c")"
...
STUBS_EOF
            ...
            if [ -f "obj/main/betaflight_SITL.elf" ]; then
                echo "Normal macOS SITL build succeeded; skipping compatibility stub relink."
            else
                # Compile stubs
